#!/usr/bin/env bash
# ShrimpBot Uninstaller
# Usage: bash uninstall.sh
#   or:  curl -fsSL https://raw.githubusercontent.com/xvirobotics/shrimpbot/main/uninstall.sh | bash
set -euo pipefail
...
echo -e "${RED}${BOLD}"
echo "  ╔══════════════════════════════════════════╗"
echo "  ║           ShrimpBot Uninstaller            ║"
echo "  ╚══════════════════════════════════════════╝"
echo -e "${NC}"
echo ""

echo -e "  This will remove ShrimpBot from: ${BOLD}${SHRIMPBOT_HOME}${NC}"
echo ""
if ! prompt_yn "Are you sure you want to uninstall ShrimpBot?"; then
...
# ============================================================================
step "Phase 1: Stopping ShrimpBot services"

if command -v pm2 &>/dev/null; then
  if pm2 describe shrimpbot &>/dev/null 2>&1; then
    info "Stopping ShrimpBot PM2 process..."
    pm2 delete shrimpbot 2>/dev/null || true
    success "ShrimpBot PM2 process removed"
  else
    info "No ShrimpBot PM2 process found"
  fi
...
# Kill any process on ShrimpBot port (default 9100)
...
for cli in mm mb shrimpbot fd; do
...
if [[ -f "$BASH_ALIASES" ]]; then
  ...
  # Remove mb() block (from "# ShrimpBot API shortcuts" to closing "}")
  if grep -q 'mb()' "$BASH_ALIASES" 2>/dev/null; then
    awk '
      /^# ShrimpBot API shortcuts/ { skip=1; next }
      skip && /^[^ \t]/ && !/^(export SHRIMPBOT|mb\(\))/ { skip=0 }
      skip { next }
      { print }
    ' "$BASH_ALIASES" > "$BASH_ALIASES.tmp" && mv "$BASH_ALIASES.tmp" "$BASH_ALIASES"
    CLEANED=true
  ...
  if [[ "$CLEANED" == "false" ]]; then
    info "No ShrimpBot shortcuts found in ~/.bash_aliases"
  fi
...
for skill in metaskill metamemory shrimpbot voice feishu-doc; do
...
# ============================================================================
# Phase 5: Remove ShrimpBot directory
# ============================================================================
step "Phase 5: Removing ShrimpBot installation"

if [[ -d "$SHRIMPBOT_HOME" ]]; then
  # Check for data that might be worth keeping
  HAS_DATA=false
  if [[ -f "$SHRIMPBOT_HOME/data/metamemory.db" ]]; then
    HAS_DATA=true
  fi

  if [[ "$HAS_DATA" == "true" ]]; then
    echo ""
    warn "MetaMemory database found at $SHRIMPBOT_HOME/data/metamemory.db"
    if prompt_yn "Back up MetaMemory data to ~/shrimpbot-backup/ before deleting?"; then
      BACKUP_DIR="$HOME/shrimpbot-backup"
      mkdir -p "$BACKUP_DIR"
      cp -r "$SHRIMPBOT_HOME/data" "$BACKUP_DIR/" 2>/dev/null || true
      [[ -f "$SHRIMPBOT_HOME/.env" ]] && cp "$SHRIMPBOT_HOME/.env" "$BACKUP_DIR/" 2>/dev/null || true
      [[ -f "$SHRIMPBOT_HOME/bots.json" ]] && cp "$SHRIMPBOT_HOME/bots.json" "$BACKUP_DIR/" 2>/dev/null || true
      success "Backed up data to $BACKUP_DIR"
    fi
  fi

  rm -rf "$SHRIMPBOT_HOME"
  success "Removed $SHRIMPBOT_HOME"
else
  info "ShrimpBot directory not found at $SHRIMPBOT_HOME"
fi
...
for dir in "$HOME/shrimpbot-workspace" "$HOME/workspace" "$HOME/projects"; do
  if [[ -d "$dir/.claude/skills/shrimpbot" ]] || [[ -d "$dir/.claude/skills/metamemory" ]]; then
    WORKSPACE_DIRS+=("$dir")
  fi
done
...
if [[ ${#WORKSPACE_DIRS[@]} -gt 0 ]]; then
  for ws in "${WORKSPACE_DIRS[@]}"; do
    echo ""
    info "Found deployed ShrimpBot skills in: $ws"
    if prompt_yn "Remove deployed skills from $ws?"; then
      for skill in metaskill metamemory shrimpbot voice; do
...
echo -e "${GREEN}${BOLD}"
echo "  ╔══════════════════════════════════════════╗"
echo "  ║        ShrimpBot — Uninstalled             ║"
echo "  ╚══════════════════════════════════════════╝"
echo -e "${NC}"
echo ""
echo -e "  ${BOLD}Removed:${NC}"
echo "    - PM2 processes (shrimpbot, metamemory)"
echo "    - CLI tools (mm, mb, shrimpbot)"
echo "    - Shell shortcuts from ~/.bash_aliases"
echo "    - Claude skills (metaskill, metamemory, shrimpbot, lark-cli skills)"
echo "    - lark-cli config (~/.lark-cli)"
echo "    - ShrimpBot directory ($SHRIMPBOT_HOME)"
if [[ -d "$HOME/shrimpbot-backup" ]]; then
  echo ""
  echo -e "  ${BOLD}Backup:${NC} ~/shrimpbot-backup/"
fi
echo ""
