...
describe('PeerManager', () => {
  ...
  it('forwardTask sends POST with X-ShrimpBot-Origin header', async () => {
  ...
  it('sends auth header when peer has secret', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ 'Authorization': 'Bearer my-secret', 'X-ShrimpBot-Origin': 'peer' }),
      }),
    );
  });
  ...
  it('does not send auth header when peer has no secret', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ 'X-ShrimpBot-Origin': 'peer' }),
      }),
    );
  });
