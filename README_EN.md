...
[![CI](https://img.shields.io/github/actions/workflow/status/xvirobotics/shrimpbot/ci.yml?branch=main&style=flat-square)](https://github.com/xvirobotics/shrimpbot/actions)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg?style=flat-square)](https://opensource.org/licenses/MIT)
[![GitHub stars](https://img.shields.io/github/stars/xvirobotics/shrimpbot?style=flat-square)](https://github.com/xvirobotics/shrimpbot)

[中文](README.md) | English | [Docs](https://xvirobotics.com/shrimpbot/)
...
![ShrimpBot Demo](resources/shrimpbot-demo.gif)

```bash
curl -fsSL https://raw.githubusercontent.com/xvirobotics/shrimpbot/main/install.sh | bash
```
...
| | ShrimpBot | Claude Code (terminal) | Dify / Coze |
|---|---|---|---|
...
![ShrimpBot Architecture](resources/shrimpbot.png)
...
Full-featured browser-based chat interface. Access at `https://your-server/web/` after starting ShrimpBot.

![ShrimpBot Web UI](resources/web-ui.png)
...
> Voice features require HTTPS. We recommend Caddy as a reverse proxy. See [Web UI docs](https://xvirobotics.com/shrimpbot/features/web-ui/).
...
3. See [WeChat Setup Guide](https://xvirobotics.com/shrimpbot/features/wechat/)
...
3. Start ShrimpBot, then enable persistent connection + `im.message.receive_v1` event
4. Publish the app

> No public IP needed. Feishu uses WebSocket, Telegram and WeChat use long polling.

**Web UI**: Visit `http://localhost:9100/web/` after starting ShrimpBot, enter your API_SECRET.

## Example Prompts

New to ShrimpBot? Here are real prompts you can send in Feishu/Telegram:
...
| `SHRIMPBOT_URL` | `http://localhost:9100` | ShrimpBot API URL. Default is local HTTP; for remote access prefer HTTPS or a private-network address |
...
| `SHRIMPBOT_PEERS` | — | Peer ShrimpBot URLs (comma-separated). Prefer HTTPS for internet-reachable peers |
...
Compatible with [cc-switch](https://github.com/farion1231/cc-switch), [cc-switch-cli](https://github.com/SaladDay/cc-switch-cli), [CCS](https://github.com/kaitranntt/ccs). Switching via `cc switch` takes effect **without restarting** ShrimpBot.
...
ShrimpBot runs Claude Code in `bypassPermissions` mode — no interactive approval:
...
The installer places `shrimpbot`, `mm`, `mb` in `~/.local/bin/` — available immediately.

```bash
# ShrimpBot management
shrimpbot update                      # pull latest, rebuild, restart
shrimpbot start / stop / restart      # PM2 management
...
CLI supports connecting to remote ShrimpBot/MetaMemory servers — configure `SHRIMPBOT_URL` and `META_MEMORY_URL` in `~/.shrimpbot/.env`.
...
```bash
git clone https://github.com/xvirobotics/shrimpbot.git
cd shrimpbot && npm install
...
ShrimpBot is built by [XVI Robotics](https://xvirobotics.com) (humanoid robot brains). We use ShrimpBot internally to run our company as an **agent-native organization** — a small team of humans supervising self-improving AI agents.

We open-sourced it because we believe this is how companies will work in the future.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=xvirobotics/shrimpbot&type=Date)](https://star-history.com/#xvirobotics/shrimpbot&Date)
