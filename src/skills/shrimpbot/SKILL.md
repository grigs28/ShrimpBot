---
name: shrimpbot
description: "ShrimpBot HTTP API for agent collaboration: talk to other bots, schedule tasks, manage bots and peers. Use when the user wants to delegate work to another bot, schedule tasks, create/remove bots, or check peer status."
---

## ShrimpBot API

ShrimpBot exposes an HTTP API for agent-to-agent collaboration, task scheduling, and bot management.
...
When you talk to a bot that isn't on the local instance, ShrimpBot automatically routes the request to the peer instance that hosts that bot. No special syntax is needed — just use `mb talk <botName> <chatId> <prompt>` as usual.
...
Auth header: `-H "Authorization: Bearer $SHRIMPBOT_API_SECRET"`
Base URL: !`echo http://localhost:${SHRIMPBOT_API_PORT:-9100}`

**Talk to a bot (primary endpoint):**
```bash
curl -s -X POST http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/talk \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET" \
...
**Create Feishu bot:**
```bash
curl -s -X POST http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/bots \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET" \
  -H "Content-Type: application/json" \
...
**Remove bot:**
```bash
curl -s -X DELETE http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/bots/<name> \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET"
```

**Update scheduled task:**
```bash
curl -s -X PATCH http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/schedule/<id> \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET" \
  -H "Content-Type: application/json" \
...
```bash
curl -s -X POST http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/schedule \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET" \
  -H "Content-Type: application/json" \
...
```bash
curl -s -X POST http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/schedule/<id>/pause \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET"
curl -s -X POST http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/schedule/<id>/resume \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET"
```
...
**List peers:**
```bash
curl -s http://localhost:${SHRIMPBOT_API_PORT:-9100}/api/peers \
  -H "Authorization: Bearer $SHRIMPBOT_API_SECRET"
```
