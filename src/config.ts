...
export interface AppConfig {
  ...
  /** Peer ShrimpBot instances for cross-instance bot discovery and task delegation. */
  peers: PeerConfig[];
}
...
function buildClaudeConfig(entry: {
  ...
  return {
    ...
    outputsBaseDir: entry.outputsBaseDir || process.env.OUTPUTS_BASE_DIR || path.join(os.tmpdir(), `shrimpbot-outputs-${os.userInfo().username}`),
    downloadsDir: entry.downloadsDir || process.env.DOWNLOADS_DIR || path.join(os.tmpdir(), `shrimpbot-downloads-${os.userInfo().username}`),
  };
}
...
function feishuBotFromEnv(): BotConfig {
  return {
    ...
    claude: {
      ...
      apiKey: undefined,
      outputsBaseDir: process.env.OUTPUTS_BASE_DIR || path.join(os.tmpdir(), `shrimpbot-outputs-${os.userInfo().username}`),
      downloadsDir: process.env.DOWNLOADS_DIR || path.join(os.tmpdir(), `shrimpbot-downloads-${os.userInfo().username}`),
    },
  };
}
...
function wechatBotFromEnv(): WechatBotConfig {
  return {
    ...
    claude: {
      ...
      model: process.env.CLAUDE_MODEL || 'claude-opus-4-7',
      apiKey: undefined,
      outputsBaseDir: expandUserPath(process.env.OUTPUTS_BASE_DIR || path.join(os.tmpdir(), `shrimpbot-outputs-${os.userInfo().username}`)),
      downloadsDir: expandUserPath(process.env.DOWNLOADS_DIR || path.join(os.tmpdir(), `shrimpbot-downloads-${os.userInfo().username}`)),
    },
  };
}
...
export function loadAppConfig(): AppConfig {
  ...
  // Expose as SHRIMPBOT_* env vars so Claude Code skills can read them via shell expansion
  process.env.SHRIMPBOT_API_PORT = String(apiPort);
  if (apiSecret) {
    process.env.SHRIMPBOT_API_SECRET = apiSecret;
  }
  ...
  if (process.env.SHRIMPBOT_PEERS) {
    const urls = process.env.SHRIMPBOT_PEERS.split(',').map((u) => u.trim()).filter(Boolean);
    const secrets = (process.env.SHRIMPBOT_PEER_SECRETS || '').split(',').map((s) => s.trim());
    const names = (process.env.SHRIMPBOT_PEER_NAMES || '').split(',').map((s) => s.trim());
