...
/**
 * Env var prefixes that may contain secrets — always strip from child process
 * environment to prevent credential leaks via `env` command or shell tools.
 * The explicit apiKey from bots.json is injected after filtering.
 */
...
function createSpawnFn(explicitApiKey?: string): (options: SpawnOptions) => SpawnedProcess {
  ...
  return (options: SpawnOptions): SpawnedProcess => {
    ...
    for (const [key, value] of Object.entries(baseEnv)) {
      ...
      // Always strip sensitive secret/token env vars from child process
      if (SENSITIVE_ENV_PREFIXES.some(p => key.startsWith(p) || key === p)) continue;
      env[key] = value;
    }
...
export class ClaudeExecutor {
  ...
  private buildQueryOptions(cwd: string, sessionId: string | undefined, abortController: AbortController, outputsDir?: string, apiContext?: ApiContext): Record<string, unknown> {
    ...
    if (apiContext) {
      ...
      // Port and secret are already set as SHRIMPBOT_* env vars in config.ts.
      appendSections.push(
        `## ShrimpBot API\nYou are running as bot "${apiContext.botName}" in chat "${apiContext.chatId}".\nUse the /shrimpbot skill for full API documentation (agent bus, scheduling, bot management).`
      );
