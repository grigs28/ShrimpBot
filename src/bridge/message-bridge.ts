...
import type { FeishuConfirm } from '../claude/feishu-confirm.js';
...
export class MessageBridge {
  ...
  private async executeQuery(msg: IncomingMessage): Promise<void> {
    ...
    metrics.setGauge('shrimpbot_active_tasks', this.runningTasks.size);
    ...
    try {
      for await (const message of executionHandle.stream) {
        ...
        if (state.status === 'waiting_for_input' && state.pendingQuestion) {
          ...
          // Send Feishu confirmation card for push notification (so user sees it on mobile)
          // The actual answer is still handled via chat reply → handleAnswer flow.
          if (this.feishuConfirm) {
            this.feishuConfirm.sendConfirmationAndWait(
              chatId,
              state.pendingQuestion,
              { botName: this.config.name, taskPrompt: displayPrompt },
            ).catch((err) => {
      ...
      metrics.incCounter('shrimpbot_tasks_total');
      metrics.incCounter('shrimpbot_tasks_by_status', lastState.status === 'complete' ? 'success' : 'error');
      metrics.observeHistogram('shrimpbot_task_duration_seconds', durationMs / 1000);
      if (lastState.costUsd) metrics.observeHistogram('shrimpbot_task_cost_usd', lastState.costUsd);
      ...
      // Send Feishu task result notification card (push notification effect)
      if (this.feishuConfirm) {
        await this.feishuConfirm.sendTaskResultNotification(chatId, {
          success: lastState.status === 'complete',
          taskPrompt: text,
          responsePreview: lastState.responseText?.slice(0, 500),
          durationMs,
      ...
      metrics.incCounter('shrimpbot_tasks_total');
      metrics.incCounter('shrimpbot_tasks_by_status', 'error');
  ...
  async executeApiTask(options: ApiTaskOptions): Promise<ApiTaskResult> {
    ...
    try {
      ...
      this.costTracker.record({ botName: this.config.name, userId, success: lastState.status === 'complete', costUsd: lastState.costUsd, durationMs });
      metrics.incCounter('shrimpbot_api_tasks_total');
      metrics.observeHistogram('shrimpbot_task_duration_seconds', durationMs / 1000);
      if (lastState.costUsd) metrics.observeHistogram('shrimpbot_task_cost_usd', lastState.costUsd);
