...
import { FeishuConfirm } from './claude/feishu-confirm.js';
...
function secureSensitiveFiles(): void {
  ...
  for (const f of files) {
    ...
    } catch { /* ignore if file doesn't exist or permission denied */ }
  }
}
...
async function startFeishuBot(botConfig: BotConfig, logger: Logger, memoryServerUrl: string, memorySecret?: string): Promise<FeishuBotHandle> {
  ...
  // Create and inject Feishu confirmation handler for interactive confirmation cards
  const feishuConfirm = new FeishuConfirm(rawSender, botLogger);
  bridge.setFeishuConfirm(feishuConfirm);
...
async function main() {
  ...
  // Secure sensitive config files on startup
  ...
  logger.info({ feishuBots: feishuCount, telegramBots: telegramCount, wechatBots: wechatCount, memoryServerUrl: appConfig.memoryServerUrl }, 'Starting ShrimpBot bridge...');

  // Create bot registry
  const registry = new BotRegistry();

  // Start bots independently so a single platform/API timeout does not
  // take down the whole ShrimpBot process.
