...
const RATE_LIMIT_MAX_REQUESTS = parseInt(process.env.API_RATE_LIMIT || '120', 10);
...
(globalThis as any).__shrimpbot_start_time = startTime;
...
export function startApiServer(options: ApiServerOptions): http.Server {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // Rate limiting (per-IP, configurable via API_RATE_LIMIT env)
    ...
    // Auth check (only /web/ static assets are exempt)
    if (secret && !url.startsWith('/web')) {
      const auth = req.headers.authorization;
      const urlToken = url.includes('token=') ? new URL(url, `http://${req.headers.host || 'localhost'}`).searchParams.get('token') : null;
      const authValid = auth?.startsWith('Bearer ') && safeCompare(auth.slice(7), secret);
      const urlTokenValid = urlToken !== null && safeCompare(urlToken, secret);
      if (!authValid && !urlTokenValid) {
