...
export class PeerManager {
  ...
  constructor(configs: PeerConfig[], logger: Logger) {
    ...
    const interval = process.env.SHRIMPBOT_PEER_POLL_INTERVAL_MS
      ? parseInt(process.env.SHRIMPBOT_PEER_POLL_INTERVAL_MS, 10)
  ...
  /** Forward a task request to a peer. Adds X-ShrimpBot-Origin header to prevent loops. */
