...
export async function handleFileRoutes(
  ...
  if (method === 'POST' && (url === '/api/upload' || url.startsWith('/api/upload?'))) {
    ...
    const uploadDir = path.join(os.tmpdir(), 'shrimpbot-uploads', chatId);
  ...
  if (method === 'GET' && url.startsWith('/api/files/preview/')) {
    ...
    const fullPath = path.resolve(path.join(os.tmpdir(), 'shrimpbot-uploads', filePart));
    const uploadsRoot = path.resolve(path.join(os.tmpdir(), 'shrimpbot-uploads'));
    ...
    try {
      ...
      if (ext === '.docx') {
        ...
        const tmpOut = path.join(os.tmpdir(), 'shrimpbot-preview-' + Date.now());
