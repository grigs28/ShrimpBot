...
export class WechatClient {
  ...
  async sendMessage(
    ...
    const clientId = opts?.clientId || `shrimpbot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  async uploadMedia(
    ...
    const filekey = `shrimpbot_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
