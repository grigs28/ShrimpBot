#!/usr/bin/env bash
# ShrimpBot Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/xvirobotics/shrimpbot/main/install.sh | bash
set -euo pipefail
...
SHRIMPBOT_REPO="${SHRIMPBOT_REPO:-https://github.com/xvirobotics/shrimpbot.git}"
...
if [[ "$OS" != "Linux" && "$OS" != "Darwin" ]]; then
  error "Unsupported OS: $OS. ShrimpBot supports Linux and macOS."
  exit 1
fi
...
# ============================================================================
step "Phase 2: Setting up ShrimpBot at ${SHRIMPBOT_HOME}"
...
if [[ -d "$SHRIMPBOT_HOME/.git" ]]; then
  ...
  if [[ "$OLD_HEAD" != "$NEW_HEAD" && -z "${SHRIMPBOT_REEXEC:-}" ]]; then
    info "install.sh updated, re-launching..."
    export SHRIMPBOT_REEXEC=1
    exec bash "$SHRIMPBOT_HOME/install.sh"
  fi
else
  info "Cloning ShrimpBot..."
  git clone "$SHRIMPBOT_REPO" "$SHRIMPBOT_HOME"
  cd "$SHRIMPBOT_HOME"
fi
success "ShrimpBot code ready at ${SHRIMPBOT_HOME}"
...
if [[ -f "$SHRIMPBOT_HOME/.env" ]]; then
  warn ".env already exists. Skipping interactive config."
  warn "Edit ${SHRIMPBOT_HOME}/.env to modify settings."
  SKIP_CONFIG=true
else
  SKIP_CONFIG=false
fi
...
if [[ "$SKIP_CONFIG" == "false" ]]; then
  ...
  prompt_input WORK_DIR "Project directory for Claude to work in" "$HOME/shrimpbot-workspace"
  mkdir -p "$WORK_DIR"
...
API_TIMEOUT_MS=600000"
  ...
  prompt_input BOT_NAME "Name for your bot" "shrimpbot"
...
if [[ "$SKIP_CONFIG" == "false" ]]; then
  # Generate .env
  {
    echo "# ShrimpBot Configuration (generated by install.sh)"
...
cp "$SHRIMPBOT_HOME/src/skills/metaskill/SKILL.md" "$SKILLS_DIR/metaskill/SKILL.md"
cp "$SHRIMPBOT_HOME/src/skills/metaskill/flows/team.md" "$SKILLS_DIR/metaskill/flows/team.md"
cp "$SHRIMPBOT_HOME/src/skills/metaskill/flows/agent.md" "$SKILLS_DIR/metaskill/flows/agent.md"
cp "$SHRIMPBOT_HOME/src/skills/metaskill/flows/skill.md" "$SKILLS_DIR/metaskill/flows/skill.md"
...
cp "$SHRIMPBOT_HOME/src/memory/skill/SKILL.md" "$SKILLS_DIR/metamemory/SKILL.md"
...
# Install shrimpbot skill (bundled in src/skills/shrimpbot/)
info "Installing shrimpbot skill..."
mkdir -p "$SKILLS_DIR/shrimpbot"
cp "$SHRIMPBOT_HOME/src/skills/shrimpbot/SKILL.md" "$SKILLS_DIR/shrimpbot/SKILL.md"
success "shrimpbot skill installed → $SKILLS_DIR/shrimpbot"

# Install voice skill (bundled in src/skills/voice/)
info "Installing voice skill..."
mkdir -p "$SKILLS_DIR/voice"
cp "$SHRIMPBOT_HOME/src/skills/voice/SKILL.md" "$SKILLS_DIR/voice/SKILL.md"
...
cp "$SHRIMPBOT_HOME/src/skills/skill-hub/SKILL.md" "$SKILLS_DIR/skill-hub/SKILL.md"
...
if [[ "$SKIP_CONFIG" == "false" && "$SETUP_FEISHU" == "true" ]]; then
  HAS_FEISHU=true
elif [[ "$SKIP_CONFIG" == "true" && -f "$SHRIMPBOT_HOME/bots.json" ]]; then
  # Detect from existing config
  if node -e "const c=JSON.parse(require('fs').readFileSync('$SHRIMPBOT_HOME/bots.json','utf-8')); process.exit((c.feishuBots||[]).length>0?0:1)" 2>/dev/null; then
    HAS_FEISHU=true
  fi
fi
...
if [[ "$SETUP_LARK_CLI" == "true" ]]; then
  ...
  if [[ ! -f "$HOME/.lark-cli/config.json" && -f "$SHRIMPBOT_HOME/bots.json" ]]; then
    FEISHU_APP_ID=$(node -e "const c=JSON.parse(require('fs').readFileSync('$SHRIMPBOT_HOME/bots.json','utf-8')); console.log((c.feishuBots||[])[0]?.feishuAppId||'')" 2>/dev/null)
    FEISHU_APP_SECRET=$(node -e "const c=JSON.parse(require('fs').readFileSync('$SHRIMPBOT_HOME/bots.json','utf-8')); console.log((c.feishuBots||[])[0]?.feishuAppSecret||'')" 2>/dev/null)
...
if [[ "$SKIP_CONFIG" == "false" ]]; then
  ...
  if [[ -f "$SHRIMPBOT_HOME/bots.json" ]]; then
    DEPLOY_WORK_DIR=$(node -e "
      const fs = require('fs');
      const cfg = JSON.parse(fs.readFileSync('$SHRIMPBOT_HOME/bots.json','utf-8'));
...
if [[ -n "${DEPLOY_WORK_DIR:-}" ]]; then
  ...
  DEPLOY_SKILLS="metaskill metamemory shrimpbot voice skill-hub"
  if [[ "$SETUP_LARK_CLI" == "true" ]]; then
  ...
  if [[ -f "$SHRIMPBOT_HOME/src/workspace/CLAUDE.md" ]]; then
    cp "$SHRIMPBOT_HOME/src/workspace/CLAUDE.md" "$DEPLOY_WORK_DIR/CLAUDE.md"
...
# ============================================================================
# Phase 7: MetaMemory (embedded in ShrimpBot)
# ============================================================================
step "Phase 7: MetaMemory"

METAMEMORY_INSTALLED=false

info "MetaMemory is embedded in ShrimpBot (no separate server needed)."
mkdir -p "${SHRIMPBOT_HOME}/data"

# Migrate existing database from standalone Python MetaMemory if found
if [[ -f "$HOME/.metamemory-data/metamemory.db" && ! -f "$SHRIMPBOT_HOME/data/metamemory.db" ]]; then
  info "Migrating existing MetaMemory database..."
  cp "$HOME/.metamemory-data/metamemory.db" "$SHRIMPBOT_HOME/data/"
...
success "MetaMemory will start automatically with ShrimpBot on port 8100"
...
# MetaMemory shortcuts (installed by ShrimpBot)
...
# Install mb() shell shortcut for ShrimpBot API (Agent Bus, Scheduling, Bot Management)
if ! grep -q 'mb()' "$BASH_ALIASES" 2>/dev/null; then
  info "Installing mb() shell shortcut..."
  cat >> "$BASH_ALIASES" << 'MBEOF'

# ShrimpBot API shortcuts (installed by ShrimpBot)
export SHRIMPBOT_URL="http://localhost:${SHRIMPBOT_API_PORT:-9100}"
export SHRIMPBOT_AUTH="Authorization: Bearer ${SHRIMPBOT_API_SECRET:-changeme}"
...
mb() {
  ...
  case "$cmd" in
    # --- Bot management ---
    bots|b)
      curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/bots" | python3 -m json.tool 2>/dev/null || curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/bots"
      ;;
    bot)
      curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/bots/$1" | python3 -m json.tool 2>/dev/null || curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/bots/$1"
      ;;
    # --- Task delegation ---
    task|t)
      ...
      curl -s -X POST "$SHRIMPBOT_URL/api/tasks" \
        -H "$SHRIMPBOT_AUTH" -H "Content-Type: application/json" \
    ...
    schedule|sched|sc)
      ...
      case "$subcmd" in
        list|ls)
          curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/schedule" | python3 -m json.tool 2>/dev/null || curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/schedule"
          ;;
        add|a)
    ...
    # --- Health ---
    health|h)
      curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/health" | python3 -m json.tool 2>/dev/null || curl -s -H "$SHRIMPBOT_AUTH" "$SHRIMPBOT_URL/api/health"
      ;;
    # --- Help ---
    *)
      echo "mb - ShrimpBot API CLI"
      echo ""
      echo "  Bots:"
...
MBEOF
  ...
  if [[ -n "${API_PORT:-}" ]]; then
    sed_i "s|\${SHRIMPBOT_API_PORT:-9100}|${API_PORT}|g" "$BASH_ALIASES"
  fi
  if [[ -n "${API_SECRET:-}" ]]; then
    sed_i "s|\${SHRIMPBOT_API_SECRET:-changeme}|${API_SECRET}|g" "$BASH_ALIASES"
  fi
...
# Install mm/mb/shrimpbot as standalone executables in ~/.local/bin (no source needed)
LOCAL_BIN="$HOME/.local/bin"
mkdir -p "$LOCAL_BIN"
CLI_TOOLS="mm mb shrimpbot"
...
success "mm/mb/shrimpbot CLI tools installed to $LOCAL_BIN"

# ============================================================================
# Phase 8: Build + Start ShrimpBot with PM2
# ============================================================================
step "Phase 8: Starting ShrimpBot"
...
if pm2 describe shrimpbot &>/dev/null 2>&1; then
  info "Removing old ShrimpBot PM2 process..."
  pm2 delete shrimpbot 2>/dev/null || true
fi
info "Starting ShrimpBot with PM2..."
pm2 start ecosystem.config.cjs

pm2 save --force 2>/dev/null || true
success "ShrimpBot is running!"
...
if [[ "$SKIP_CONFIG" == "false" && "${SETUP_WECHAT:-false}" == "true" ]]; then
  ...
  if node -e "const c=JSON.parse(require('fs').readFileSync('$SHRIMPBOT_HOME/bots.json','utf-8')); process.exit((c.wechatBots||[]).length>0?0:1)" 2>/dev/null; then
    HAS_WECHAT_BOT=true
  fi
fi
...
if [[ "$HAS_WECHAT_BOT" == "true" ]]; then
  ...
  WX_QR_URL=""
  LOG_FILE="$SHRIMPBOT_HOME/logs/out.log"
  ...
  if [[ -n "$WX_QR_URL" ]]; then
    ...
    echo "    pm2 logs shrimpbot --lines 30"
  fi
fi
...
echo -e "${GREEN}${BOLD}"
echo "  ╔══════════════════════════════════════════╗"
echo "  ║           ShrimpBot — Ready!               ║"
echo "  ╚══════════════════════════════════════════╝"
echo -e "${NC}"
echo ""
echo -e "  ${BOLD}Installation:${NC}   ${SHRIMPBOT_HOME}"
if [[ "${SKIP_CONFIG}" == "false" ]]; then
...
echo "    pm2 logs shrimpbot          # View ShrimpBot logs"
echo "    pm2 restart shrimpbot       # Restart ShrimpBot"
echo "    pm2 stop shrimpbot          # Stop ShrimpBot"
if [[ "$METAMEMORY_INSTALLED" == "true" ]]; then
...
if [[ "${SKIP_CONFIG}" == "false" ]]; then
  ...
  if [[ "${SETUP_WECHAT:-false}" == "true" ]]; then
    echo "    ${STEP_NUM}. Scan the WeChat QR code shown above (or check pm2 logs shrimpbot)"
    STEP_NUM=$((STEP_NUM + 1))
    echo "    ${STEP_NUM}. Send a message to your ClawBot in WeChat"
    STEP_NUM=$((STEP_NUM + 1))
  fi
  echo "    ${STEP_NUM}. Check logs: pm2 logs shrimpbot"
fi
echo ""
