/* ============================================================
   ShrimpBot Web — Global State (Zustand)
   ============================================================ */
...
function persistSessions(sessions: Map<string, ChatSession>) {
  try {
    ...
    localStorage.setItem('shrimpbot:sessions', JSON.stringify(obj));
  } catch {
...
function loadSessions(): Map<string, ChatSession> {
  try {
    const raw = localStorage.getItem('shrimpbot:sessions');
...
export const useStore = create<AppStore>((set, get) => ({
  /* ---- Auth ---- */
  token: localStorage.getItem('shrimpbot:token'),

  login(token: string) {
    localStorage.setItem('shrimpbot:token', token);
    set({ token });
  },

  logout() {
    localStorage.removeItem('shrimpbot:token');
  ...
  clearSessions() {
    localStorage.removeItem('shrimpbot:sessions');
  ...
  /* ---- Theme ---- */
  theme: (localStorage.getItem('shrimpbot:theme') as Theme) || 'dark',
  toggleTheme() {
    const next = get().theme === 'dark' ? 'light' : 'dark';
    localStorage.setItem('shrimpbot:theme', next);
    document.documentElement.setAttribute('data-theme', next);
    set({ theme: next });
  },

  /* ---- Font size ---- */
  fontSize: (localStorage.getItem('shrimpbot:fontsize') as 'small' | 'normal' | 'large' | 'xl') || 'normal',
  setFontSize(size: 'small' | 'normal' | 'large' | 'xl') {
    const scales: Record<string, string> = { small: '0.9', normal: '1', large: '1.1', xl: '1.25' };
    localStorage.setItem('shrimpbot:fontsize', size);
  ...
  teamViewMode: (localStorage.getItem('shrimpbot:teamViewMode') as 'workspace' | 'office') || 'workspace',
  setTeamViewMode(mode: 'workspace' | 'office') {
    localStorage.setItem('shrimpbot:teamViewMode', mode);
