# ShrimpBot Installer for Windows PowerShell
# Usage: irm https://raw.githubusercontent.com/xvirobotics/shrimpbot/main/install.ps1 | iex
...
# ============================================================================
$ShrimpbotRepo = if ($env:SHRIMPBOT_REPO) { $env:SHRIMPBOT_REPO } else { "https://github.com/xvirobotics/shrimpbot.git" }
$ShrimpbotHome = if ($env:SHRIMPBOT_HOME) { $env:SHRIMPBOT_HOME } else { Join-Path $env:USERPROFILE "shrimpbot" }
...
function Write-Banner {
    ...
    Write-Host "  |            ShrimpBot Installer                |" -ForegroundColor Cyan
...
# ============================================================================
Write-Step "Phase 2: Setting up ShrimpBot at $ShrimpbotHome"

if (Test-Path (Join-Path $ShrimpbotHome ".git")) {
    Write-Info "Existing installation found, pulling latest..."
    Push-Location $ShrimpbotHome
    ...
    if ($OldHead -ne $NewHead -and -not $env:SHRIMPBOT_REEXEC) {
        Write-Info "install.ps1 updated, re-launching..."
        $env:SHRIMPBOT_REEXEC = "1"
        & (Join-Path $ShrimpbotHome "install.ps1")
        Pop-Location
        exit 0
    }
    Pop-Location
} else {
    Write-Info "Cloning ShrimpBot..."
    git clone $ShrimpbotRepo $ShrimpbotHome
}
Write-Success "ShrimpBot code ready at $ShrimpbotHome"
...
$EnvFile = Join-Path $ShrimpbotHome ".env"
$SkipConfig = $false
...
if (-not $SkipConfig) {
    ...
    $DefaultWorkDir = Join-Path $env:USERPROFILE "shrimpbot-workspace"
    ...
    $BotName = Read-Input "Name for your bot" "shrimpbot"
...
if (-not $SkipConfig) {
    ...
    $envContent = @"
# ShrimpBot Configuration (generated by install.ps1)
# $timestamp
...
"@
    ...
    $BotsJson = Join-Path $ShrimpbotHome "bots.json"
    $FeishuBotsJson = "[]"
    $TelegramBotsJson = "[]"
...
Copy-Item (Join-Path $ShrimpbotHome "src\skills\metaskill\SKILL.md") (Join-Path $SkillsDir "metaskill\SKILL.md") -Force
Copy-Item (Join-Path $ShrimpbotHome "src\skills\metaskill\flows\team.md") (Join-Path $SkillsDir "metaskill\flows\team.md") -Force
Copy-Item (Join-Path $ShrimpbotHome "src\skills\metaskill\flows\agent.md") (Join-Path $SkillsDir "metaskill\flows\agent.md") -Force
Copy-Item (Join-Path $ShrimpbotHome "src\skills\metaskill\flows\skill.md") (Join-Path $SkillsDir "metaskill\flows\skill.md") -Force
...
Copy-Item (Join-Path $ShrimpbotHome "src\memory\skill\SKILL.md") (Join-Path $SkillsDir "metamemory\SKILL.md") -Force
...
# Install shrimpbot skill
Write-Info "Installing shrimpbot skill..."
New-Item -ItemType Directory -Path (Join-Path $SkillsDir "shrimpbot") -Force | Out-Null
Copy-Item (Join-Path $ShrimpbotHome "src\skills\shrimpbot\SKILL.md") (Join-Path $SkillsDir "shrimpbot\SKILL.md") -Force
Write-Success "shrimpbot skill installed -> $(Join-Path $SkillsDir 'shrimpbot')"

# Install voice skill
Write-Info "Installing voice skill..."
New-Item -ItemType Directory -Path (Join-Path $SkillsDir "voice") -Force | Out-Null
Copy-Item (Join-Path $ShrimpbotHome "src\skills\voice\SKILL.md") (Join-Path $SkillsDir "voice\SKILL.md") -Force
...
Copy-Item (Join-Path $ShrimpbotHome "src\skills\skill-hub\SKILL.md") (Join-Path $SkillsDir "skill-hub\SKILL.md") -Force
...
if (-not $SkipConfig -and $SetupFeishu) {
    $HasFeishu = $true
} elseif ($SkipConfig -and (Test-Path (Join-Path $ShrimpbotHome "bots.json"))) {
    try {
        $result = node -e "const c=JSON.parse(require('fs').readFileSync('$(Join-Path $ShrimpbotHome "bots.json")','utf-8'));process.exit((c.feishuBots||[]).length>0?0:1)" 2>$null
        if ($LASTEXITCODE -eq 0) { $HasFeishu = $true }
    } catch {}
}
$feishuDocSkill = Join-Path $ShrimpbotHome "src\skills\feishu-doc\SKILL.md"
...
if (-not $SkipConfig) {
    ...
    $botsJsonPath = Join-Path $ShrimpbotHome "bots.json"
...
if ($DeployWorkDir) {
    ...
    $deploySkills = @("metaskill", "metamemory", "shrimpbot", "voice", "skill-hub")
    ...
    $workspaceClaude = Join-Path $ShrimpbotHome "src\workspace\CLAUDE.md"
...
$cliTools = @("mm", "mb", "shrimpbot")
if ($HasFeishu) { $cliTools += "fd" }

if ($HasBash) {
    foreach ($cli in $cliTools) {
        $srcScript = Join-Path $ShrimpbotHome "bin\$cli"
    ...
    if ($HasFeishu) {
        Write-Success "mm/mb/shrimpbot/fd CLI tools installed to $LocalBin (with .cmd wrappers)"
    } else {
        Write-Success "mm/mb/shrimpbot CLI tools installed to $LocalBin (with .cmd wrappers)"
    }
} else {
    Write-Warn "Git Bash not found. CLI tools (mm, mb, shrimpbot) require bash."
...
Write-Info "MetaMemory is embedded in ShrimpBot (no separate server needed)."
New-Item -ItemType Directory -Path (Join-Path $ShrimpbotHome "data") -Force | Out-Null
...
$NewDb = Join-Path $ShrimpbotHome "data\metamemory.db"
...
Write-Success "MetaMemory will start automatically with ShrimpBot on port 8100"

# ============================================================================
# Phase 8: Build + Start ShrimpBot with PM2
# ============================================================================
Write-Step "Phase 8: Starting ShrimpBot"
...
try {
    $pm2Desc = pm2 describe shrimpbot 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Info "Removing old ShrimpBot PM2 process..."
        pm2 delete shrimpbot 2>$null
    }
} catch {}

Write-Info "Starting ShrimpBot with PM2..."
pm2 start ecosystem.config.cjs

try { pm2 save --force 2>$null } catch {}
Write-Success "ShrimpBot is running!"
Pop-Location
...
Write-Host "  |           ShrimpBot -- Ready!                |" -ForegroundColor Green
Write-Host "  +============================================+" -ForegroundColor Green
Write-Host ""

Write-Host "  Installation:   " -ForegroundColor White -NoNewline; Write-Host $ShrimpbotHome
if (-not $SkipConfig) {
...
Write-Host "    pm2 logs shrimpbot          # View ShrimpBot logs"
Write-Host "    pm2 restart shrimpbot       # Restart ShrimpBot"
Write-Host "    pm2 stop shrimpbot          # Stop ShrimpBot"
if ($MetamemoryInstalled) {
...
if (-not $SkipConfig) {
    ...
    Write-Host "    $StepNum. Check logs: pm2 logs shrimpbot"
}
Write-Host ""
