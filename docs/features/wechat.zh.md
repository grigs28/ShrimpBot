...
ShrimpBot 支持通过微信 ClawBot 插件（iLink Bot API）接入个人微信，让你直接在微信里与 Claude Code Agent 对话。
...
在微信中给 ClawBot 发消息，ShrimpBot 会自动处理并回复。
...
!!! note "消息更新方式"
    微信不支持编辑已发送的消息。ShrimpBot 处理请求时会每隔 5 秒发送工具调用进度（如 `🔧 运行中... ✓ Read file.ts`），完成后发送最终结果作为新消息。
...
删除 `data/wechat-tokens.json` 中对应的条目，重启 ShrimpBot 重新扫码。
