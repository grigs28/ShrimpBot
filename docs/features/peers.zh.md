...
跨实例 Bot 发现和任务路由。连接多个 ShrimpBot 实例 — 同机或远程服务器。

## 概述

Peers 实现了**联邦架构**，多个 ShrimpBot 实例可以互相发现 Bot 并自动路由任务。适用于：

- 同一台机器上多个用户运行各自的 ShrimpBot 实例
- 团队在不同服务器上部署 ShrimpBot
- 跨环境共享专用 Bot
...
4. **防循环** — 转发请求带 `X-ShrimpBot-Origin` header 防止循环委派
...
=== "环境变量 (.env)"
    ...
    - `SHRIMPBOT_PEERS` — 逗号分隔的 peer URL 列表（必填）
    - `SHRIMPBOT_PEER_SECRETS` — 逗号分隔的密钥，按位置与 URL 对应（可选，peer 设置了 `API_SECRET` 时需要）
    - `SHRIMPBOT_PEER_NAMES` — 逗号分隔的显示名称（可选，不填会从 URL 自动推导，如 `localhost-9200`）
...
!!! tip "不需要 bots.json"
    如果你只运行一个 Bot，直接在 `.env` 加 `SHRIMPBOT_PEERS` 就行，不需要 `bots.json`。`bots.json` 的 peers 字段只是多 Bot 配置的便利选项。
