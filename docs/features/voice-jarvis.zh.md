...
通过 AirPods 和 Siri 免手免屏与任意 ShrimpBot Agent 语音交流。无需安装 App，只用 iOS 快捷指令。
...
"Hey Siri, Jarvis"
  ...
  Siri 听写语音 → 文字
        ↓
  HTTP POST 到 ShrimpBot /api/talk
        ↓
...
"Hey Siri, Jarvis"
  ...
  录制音频（原始音频捕获）
        ↓
  HTTP POST 音频到 ShrimpBot /api/voice
        ↓
...
- ShrimpBot 服务器可从外网访问（公网 IP + 9100 端口开放）
- ShrimpBot `.env` 中的 `API_SECRET`
- ShrimpBot `.env` 中设置 `VOLCENGINE_TTS_APPID` + `VOLCENGINE_TTS_ACCESS_KEY`（推荐，用于豆包 STT+TTS），或 `OPENAI_API_KEY`（用于 Whisper STT 备选）
...
- 需要网络连接（Siri + Whisper STT + ShrimpBot API）
