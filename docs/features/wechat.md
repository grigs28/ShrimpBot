...
ShrimpBot supports WeChat personal accounts via the ClawBot plugin (iLink Bot API), allowing you to chat with Claude Code agents directly in WeChat.
...
- ShrimpBot installed and ready to run
...
2. Start ShrimpBot and wait for QR login
...
Send a message to ClawBot in WeChat — ShrimpBot handles it and replies.
...
!!! note "Message Updates"
    WeChat doesn't support editing sent messages. ShrimpBot sends tool progress updates every 5 seconds (e.g. `🔧 Running... ✓ Read file.ts`), then sends the final result as a new message.
...
**Q: Token expired?**
Delete the entry from `data/wechat-tokens.json` and restart ShrimpBot to re-scan.
