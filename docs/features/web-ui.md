...
A full-featured browser-based chat interface for ShrimpBot with real-time streaming, voice call mode, and MetaMemory browsing.

## Overview

The Web UI is a React SPA served at `/web/` on your ShrimpBot server. It connects to any configured bot via WebSocket and provides the same functionality as the Feishu/Telegram chat — plus phone call voice mode.
...
1. Start ShrimpBot: `npm run dev` or `shrimpbot start`
...
Add an A record for your domain (e.g. `shrimpbot.yourdomain.com`) pointing to your server's public IP. Wait for DNS propagation:

```bash
host shrimpbot.yourdomain.com 1.1.1.1
```
...
Open `https://shrimpbot.yourdomain.com/web/` in a browser. The phone call button now has microphone access.
...
This starts Vite on port 5173 with proxy to ShrimpBot on port 9100.
