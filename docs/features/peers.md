...
Cross-instance bot discovery and task routing. Connect multiple ShrimpBot instances — on the same machine or across remote servers.

## Overview

Peers enables a **federated architecture** where multiple ShrimpBot instances discover each other's bots and route tasks automatically. This is useful when:

- Multiple users on the same machine run separate ShrimpBot instances
- Teams deploy ShrimpBot on different servers
...
4. **Anti-loop** — Forwarded requests carry `X-ShrimpBot-Origin` header to prevent circular delegation
...
=== "Environment Variables (.env)"
    ...
    ```bash
    SHRIMPBOT_PEERS=http://localhost:9200,http://192.168.1.50:9100
    SHRIMPBOT_PEER_SECRETS=alice-secret,bob-secret
    SHRIMPBOT_PEER_NAMES=alice,bob
    ```

    - `SHRIMPBOT_PEERS` — comma-separated peer URLs (required)
    - `SHRIMPBOT_PEER_SECRETS` — comma-separated secrets, positional match with URLs (optional, needed if the peer has `API_SECRET` set)
    - `SHRIMPBOT_PEER_NAMES` — comma-separated display names (optional, auto-derived from URL if omitted, e.g. `localhost-9200`)
...
!!! tip "You don't need bots.json"
    If you're running a single bot, just add `SHRIMPBOT_PEERS` to your `.env` — no `bots.json` needed. The `bots.json` peers field is only a convenience for multi-bot setups.
