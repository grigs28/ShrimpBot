...
| Variable | Default | Description |
|----------|---------|-------------|
| `SHRIMPBOT_PEERS` | — | Comma-separated peer URLs. Prefer HTTPS for internet-reachable peers; use plain HTTP only for localhost or a private overlay network |
| `SHRIMPBOT_PEER_SECRETS` | — | Comma-separated peer secrets (positional match) |
| `SHRIMPBOT_PEER_NAMES` | auto | Comma-separated peer names |
| `SHRIMPBOT_PEER_POLL_INTERVAL_MS` | `30000` | Peer poll interval |

## Remote Access

| Variable | Default | Description |
|----------|---------|-------------|
| `SHRIMPBOT_URL` | `http://localhost:9100` | ShrimpBot API URL for CLI. The default is local HTTP; for remote access prefer an HTTPS reverse proxy or a private-network address |
...
ShrimpBot supports any Anthropic-compatible API:
