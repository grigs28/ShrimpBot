...
| 变量 | 默认 | 说明 |
|------|------|------|
| `SHRIMPBOT_PEERS` | — | 逗号分隔的 peer URL |
| `SHRIMPBOT_PEER_SECRETS` | — | 逗号分隔的 peer secret（位置对应） |
| `SHRIMPBOT_PEER_NAMES` | 自动 | 逗号分隔的 peer 名称 |
| `SHRIMPBOT_PEER_POLL_INTERVAL_MS` | `30000` | peer 拉取间隔 |

## 远程访问

| 变量 | 默认 | 说明 |
|------|------|------|
| `SHRIMPBOT_URL` | `http://localhost:9100` | ShrimpBot API 地址（CLI 用） |
