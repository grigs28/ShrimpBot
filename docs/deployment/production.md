...
shrimpbot update                      # pull + rebuild + restart
```
...
This registers ShrimpBot as a system service that starts automatically after reboot.

## Manual PM2 Commands

```bash
pm2 start ecosystem.config.cjs      # start
pm2 restart shrimpbot                  # restart
pm2 stop shrimpbot                     # stop
pm2 logs shrimpbot                     # view logs
...
```bash
# In ~/.shrimpbot/.env
SHRIMPBOT_URL=https://shrimpbot.yourdomain.com
...
Caddy automatically obtains and renews certificates. WebSocket connections (`/ws`) are proxied transparently — no additional configuration needed. Use the same HTTPS hostnames for `SHRIMPBOT_URL`, `META_MEMORY_URL`, and remote peer entries in `SHRIMPBOT_PEERS`.
