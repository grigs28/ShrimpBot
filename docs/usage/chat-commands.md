...
Commands you can send to ShrimpBot in Feishu or Telegram.
...
| `/shrimpbot` | Load Agent Bus docs (scheduling, bot management, cross-instance talk) |
...
- `/metaskill` and `/shrimpbot` are skills that get loaded into Claude's context on demand
...
Feishu doesn't allow @mentioning while uploading files or images (especially on mobile). ShrimpBot supports **upload first, @mention later**:
...
@ShrimpBot analyze these files   ← then @Bot, files auto-attached
```
