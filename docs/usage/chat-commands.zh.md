...
在飞书或 Telegram 中发送给 ShrimpBot 的命令。
...
| `/shrimpbot` | 加载 Agent 总线文档（调度、Bot 管理、跨实例对话） |
...
- `/metaskill` 和 `/shrimpbot` 是按需加载到 Claude 上下文的 skill
...
飞书上传文件/图片时无法同时 @Bot（尤其是手机端）。ShrimpBot 支持 **先传后 @**：
...
@ShrimpBot 帮我分析这些文件   ← 再 @Bot，文件自动附上
```
