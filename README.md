...
中文 | [English](README_EN.md) | [文档站](https://xvirobotics.com/shrimpbot/zh/)
...
| | ShrimpBot | 直接用 Claude Code | Dify / Coze |
|---|---|---|---|
...
![ShrimpBot 架构图](resources/shrimpbot.png)
...
ShrimpBot 支持 4 种方式与你的 Agent 团队交互：
...
> 语音功能需要 HTTPS。推荐用 Caddy 反向代理，自动管理证书。详见 [Web UI 文档](https://xvirobotics.com/shrimpbot/zh/features/web-ui/)。
...
3. 详见 [微信接入指南](https://xvirobotics.com/shrimpbot/zh/features/wechat/)

### 飞书

1. [open.feishu.cn](https://open.feishu.cn/) 创建应用 → 添加「机器人」能力
2. 开通权限：`im:message`、`im:message:readonly`、`im:resource`、`im:chat:readonly`
3. 先启动 ShrimpBot，再开启「长连接」+ `im.message.receive_v1` 事件
4. 发布应用

> 不需要公网 IP。飞书用 WebSocket，Telegram 和微信用长轮询。

**Web UI**：启动 ShrimpBot 后访问 `http://localhost:9100/web/`，输入 API_SECRET 即可使用。

## 示例 Prompt

刚接触 ShrimpBot？以下是你可以直接在飞书/Telegram 中发送的真实 prompt：
...
| `SHRIMPBOT_URL` | `http://localhost:9100` | ShrimpBot API 地址 |
| `META_MEMORY_URL` | `http://localhost:8100` | MetaMemory 服务地址 |
| `SHRIMPBOT_PEERS` | — | Peer ShrimpBot 地址（逗号分隔） |
| `LOG_LEVEL` | info | 日志级别 |
...
兼容 [cc-switch](https://github.com/farion1231/cc-switch)、[cc-switch-cli](https://github.com/SaladDay/cc-switch-cli)、[CCS](https://github.com/kaitranntt/ccs) 等认证切换工具。用 `cc switch` 切换 API/订阅模式后，ShrimpBot **无需重启**即可生效。
...
ShrimpBot 以 `bypassPermissions` 模式运行 Claude Code — 无交互式确认：
...
安装器将 `shrimpbot`、`mm`、`mb` 放到 `~/.local/bin/`，安装后立即可用。

```bash
# ShrimpBot 管理
shrimpbot update                      # 拉取最新代码，重新构建，重启
shrimpbot start / stop / restart      # PM2 管理
...
CLI 支持连接远程 ShrimpBot/MetaMemory 服务器，在 `~/.shrimpbot/.env` 配置 `SHRIMPBOT_URL` 和 `META_MEMORY_URL` 即可。
...
ShrimpBot 由 [XVI Robotics](https://xvirobotics.com) 打造（人形机器人大脑公司）。我们在内部用 ShrimpBot 把公司打造成 **Agent Native 组织** —— 一个小团队的人类，监督自我进化的 AI Agent。
