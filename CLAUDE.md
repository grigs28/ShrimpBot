...
ShrimpBot — A bridge service that connects IM bots (Feishu/Lark) to the Claude Code Agent SDK. Users chat with Claude Code from Feishu (including mobile), with real-time streaming updates via interactive cards. Runs Claude in `bypassPermissions` mode since there's no terminal for interactive approval.
...
- **`src/api/peer-manager.ts`** — Manages cross-instance bot discovery and task forwarding. Polls peer ShrimpBot instances every 30s, caches their bot lists, supports qualified name routing (`peerName/botName`). Anti-loop via `X-ShrimpBot-Origin` header.
...
1. **Per-chat outputs directory** — Before each execution, a fresh directory is created at `/tmp/shrimpbot-outputs/<chatId>/`.
...
A centralized skill registry that allows bots to publish, discover, and install skills across ShrimpBot instances.
...
**Dev workflow**: Run `cd web && npm run dev` for Vite dev server (port 5173) with API/WS proxy to 9100. Production: `npm run build:web` builds to `dist/web/`, served by ShrimpBot's HTTP server.
...
Add an A record for your domain (e.g. `shrimpbot.xvirobotics.com`) pointing to your server's public IP. Wait for DNS propagation (check with `host <domain> 1.1.1.1`).

### Step 3: Configure Caddy

```bash
sudo tee /etc/caddy/Caddyfile > /dev/null << 'EOF'
shrimpbot.yourdomain.com {
...
Open `https://shrimpbot.yourdomain.com/web/` in a browser. The phone call button in Chat now has microphone access.
...
echo 'ANTHROPIC_API_KEY=sk-ant-your-key' >> /path/to/shrimpbot/.env
```
...
cd /path/to/shrimpbot && npm run dev
```
